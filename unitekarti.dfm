object frmUnite: TfrmUnite
  Left = 287
  Top = 293
  BorderIcons = [biSystemMenu]
  BorderStyle = bsSingle
  Caption = 'Ünite Kartý Giriþi'
  ClientHeight = 172
  ClientWidth = 483
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  Menu = MainMenu1
  OldCreateOrder = False
  Position = poScreenCenter
  OnClose = FormClose
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object ToolBar1: TToolBar
    Left = 0
    Top = 0
    Width = 483
    Height = 29
    Caption = 'ToolBar1'
    Flat = True
    Images = ImageList1
    TabOrder = 0
    object tbtnYENI: TToolButton
      Left = 0
      Top = 0
      Caption = 'Yeni'
      ImageIndex = 0
      MenuItem = Yeni1
    end
    object tbtnKAYDET: TToolButton
      Left = 23
      Top = 0
      Caption = 'Kaydet'
      ImageIndex = 1
      MenuItem = Kaydet1
    end
    object tbtnSIL: TToolButton
      Left = 46
      Top = 0
      Caption = 'Sil'
      ImageIndex = 2
      MenuItem = Sil1
    end
  end
  object PageControl1: TPageControl
    Left = 0
    Top = 28
    Width = 483
    Height = 144
    ActivePage = TabSheet1
    TabOrder = 1
    object TabSheet1: TTabSheet
      Caption = 'Ünite Kartý'
      object GroupBox1: TGroupBox
        Left = 2
        Top = -1
        Width = 471
        Height = 116
        TabOrder = 0
        object Label3: TLabel
          Left = 240
          Top = 14
          Width = 49
          Height = 13
          Caption = 'Kýsým Amiri'
        end
        object Label2: TLabel
          Left = 11
          Top = 13
          Width = 43
          Height = 13
          Caption = 'Ünite Adý'
        end
        object Bevel1: TBevel
          Left = 231
          Top = 6
          Width = 2
          Height = 108
        end
        object dbcomboKisimAmiri: TDBComboBox
          Left = 240
          Top = 31
          Width = 164
          Height = 21
          DataField = 'KISIM_AMIRI_ADI'
          DataSource = datamoduleORTEG.dsrcUNITE
          ItemHeight = 13
          TabOrder = 0
          OnClick = dbcomboKisimAmiriClick
        end
        object dbeditKsARutbesi: TDBEdit
          Left = 240
          Top = 57
          Width = 222
          Height = 21
          DataField = 'KISIM_AMIRI_RUTBESI'
          DataSource = datamoduleORTEG.dsrcUNITE
          Enabled = False
          TabOrder = 1
        end
        object dbeditKsAGorevi: TDBEdit
          Left = 240
          Top = 83
          Width = 222
          Height = 21
          DataField = 'KISIM_AMIRI_GOREVI'
          DataSource = datamoduleORTEG.dsrcUNITE
          Enabled = False
          TabOrder = 2
          OnKeyPress = dbeditKsAGoreviKeyPress
        end
        object comboUniteAdi: TComboBox
          Left = 12
          Top = 29
          Width = 211
          Height = 21
          ItemHeight = 13
          TabOrder = 3
          OnClick = comboUniteAdiClick
          OnKeyPress = comboUniteAdiKeyPress
        end
      end
    end
    object TabSheet3: TTabSheet
      Caption = 'Üniteden Ýadeler'
      ImageIndex = 2
      object DBGrid2: TDBGrid
        Left = 2
        Top = 2
        Width = 470
        Height = 113
        DataSource = dsrcGIRISLER
        Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'MS Sans Serif'
        TitleFont.Style = []
        Columns = <
          item
            Expanded = False
            FieldName = 'TARIH'
            Title.Alignment = taCenter
            Title.Caption = 'Tarih'
            Width = 60
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'BELGE'
            Title.Caption = 'Blg. No'
            Width = 43
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'STOK_ADI'
            Title.Alignment = taCenter
            Title.Caption = 'Cinsi'
            Width = 103
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'MIKTAR'
            Title.Alignment = taRightJustify
            Title.Caption = 'Miktar'
            Width = 50
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'BIRIMI'
            Title.Alignment = taCenter
            Title.Caption = 'Birim'
            Width = 34
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'ALIS_FIYATI'
            Title.Alignment = taRightJustify
            Title.Caption = 'Alýþ Fiyatý'
            Width = 70
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'SATIS_FIYATI'
            Title.Alignment = taRightJustify
            Title.Caption = 'Satýþ Fiyatý'
            Width = 70
            Visible = True
          end>
      end
    end
    object TabSheet4: TTabSheet
      Caption = 'Üniteye Giriþler'
      ImageIndex = 3
      object DBGrid3: TDBGrid
        Left = 2
        Top = 2
        Width = 470
        Height = 113
        DataSource = dsrcCIKISLAR
        Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'MS Sans Serif'
        TitleFont.Style = []
        Columns = <
          item
            Expanded = False
            FieldName = 'TARIH'
            Title.Alignment = taCenter
            Title.Caption = 'Tarih'
            Width = 60
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'BELGE'
            Title.Caption = 'Blg. No'
            Width = 43
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'STOK_ADI'
            Title.Alignment = taCenter
            Title.Caption = 'Cinsi'
            Width = 103
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'MIKTAR'
            Title.Alignment = taRightJustify
            Title.Caption = 'Miktar'
            Width = 50
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'BIRIMI'
            Title.Alignment = taCenter
            Title.Caption = 'Birim'
            Width = 34
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'ALIS_FIYATI'
            Title.Alignment = taRightJustify
            Title.Caption = 'Alýþ Fiyatý'
            Width = 70
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'SATIS_FIYATI'
            Title.Alignment = taRightJustify
            Title.Caption = 'Satýþ Fiyatý'
            Width = 70
            Visible = True
          end>
      end
    end
  end
  object MainMenu1: TMainMenu
    Images = ImageList1
    Left = 74
    Top = 135
    object Dosya1: TMenuItem
      Caption = 'Dosya'
      object Yeni1: TMenuItem
        Caption = 'Yeni'
        ImageIndex = 0
        OnClick = Yeni1Click
      end
      object Kaydet1: TMenuItem
        Caption = 'Kaydet'
        ImageIndex = 1
        OnClick = Kaydet1Click
      end
      object Sil1: TMenuItem
        Caption = 'Sil'
        ImageIndex = 2
        OnClick = Sil1Click
      end
      object N1: TMenuItem
        Caption = '-'
      end
      object k1: TMenuItem
        Caption = 'Çýkýþ'
      end
    end
    object Rapor1: TMenuItem
      Caption = 'Rapor'
      object KartDkm1: TMenuItem
        Caption = 'Kart Dökümü'
      end
    end
    object Yardm1: TMenuItem
      Caption = 'Yardým'
      object erik1: TMenuItem
        Caption = 'Ýçerik'
        HelpContext = 5
        ShortCut = 112
        OnClick = erik1Click
      end
    end
  end
  object ImageList1: TImageList
    Left = 12
    Top = 135
    Bitmap = {
      
      000000000000}
  end
  object Timer1: TTimer
    Enabled = False
    Interval = 100
    OnTimer = Timer1Timer
    Left = 44
    Top = 135
  end
  object dsrcGIRISLER: TDataSource
    DataSet = queryGIRISLER
    Left = 105
    Top = 135
  end
  object dsrcCIKISLAR: TDataSource
    DataSet = queryCIKISLAR
    Left = 137
    Top = 135
  end
  object queryGIRISLER: TQuery
    DatabaseName = 'ORTEG2002'
    DataSource = datamoduleORTEG.dsrcUNITE
    SQL.Strings = (
      'select * from stokhar'
      'where belge_turu='#39'GIRDI'#39' and firma_adi=:unite_adi'
      'order by tarih desc')
    Left = 105
    Top = 105
    ParamData = <
      item
        DataType = ftString
        Name = 'UNITE_ADI'
        ParamType = ptUnknown
      end>
    object queryGIRISLERBELGE_TURU: TStringField
      FieldName = 'BELGE_TURU'
      Origin = 'ORTEG2002."stokhar.DB".BELGE_TURU'
      Size = 10
    end
    object queryGIRISLERFIRMA_KODU: TFloatField
      FieldName = 'FIRMA_KODU'
      Origin = 'ORTEG2002."stokhar.DB".FIRMA_KODU'
    end
    object queryGIRISLERFIRMA_ADI: TStringField
      FieldName = 'FIRMA_ADI'
      Origin = 'ORTEG2002."stokhar.DB".FIRMA_ADI'
      Size = 35
    end
    object queryGIRISLERTARIH: TDateField
      FieldName = 'TARIH'
      Origin = 'ORTEG2002."stokhar.DB".TARIH'
    end
    object queryGIRISLERBELGE: TStringField
      FieldName = 'BELGE'
      Origin = 'ORTEG2002."stokhar.DB".BELGE'
      Size = 15
    end
    object queryGIRISLERIC_DAGITIM_NO: TStringField
      FieldName = 'IC_DAGITIM_NO'
      Origin = 'ORTEG2002."stokhar.DB".IC_DAGITIM_NO'
      Size = 15
    end
    object queryGIRISLERSTOK_ADI: TStringField
      FieldName = 'STOK_ADI'
      Origin = 'ORTEG2002."stokhar.DB".STOK_ADI'
      Size = 30
    end
    object queryGIRISLERBIRIMI: TStringField
      FieldName = 'BIRIMI'
      Origin = 'ORTEG2002."stokhar.DB".BIRIMI'
      Size = 15
    end
    object queryGIRISLERMIKTAR: TFloatField
      FieldName = 'MIKTAR'
      Origin = 'ORTEG2002."stokhar.DB".MIKTAR'
    end
    object queryGIRISLERBIRIM2: TStringField
      FieldName = 'BIRIM2'
      Origin = 'ORTEG2002."stokhar.DB".BIRIM2'
      Size = 15
    end
    object queryGIRISLERMIKTAR2: TFloatField
      FieldName = 'MIKTAR2'
      Origin = 'ORTEG2002."stokhar.DB".MIKTAR2'
    end
    object queryGIRISLERALIS_FIYATI: TFloatField
      FieldName = 'ALIS_FIYATI'
      Origin = 'ORTEG2002."stokhar.DB".ALIS_FIYATI'
      DisplayFormat = '###,###,###,###,###'
    end
    object queryGIRISLERALIS_TUTARI: TFloatField
      FieldName = 'ALIS_TUTARI'
      Origin = 'ORTEG2002."stokhar.DB".ALIS_TUTARI'
      DisplayFormat = '###,###,###,###,###'
    end
    object queryGIRISLERSATIS_FIYATI: TFloatField
      FieldName = 'SATIS_FIYATI'
      Origin = 'ORTEG2002."stokhar.DB".SATIS_FIYATI'
      DisplayFormat = '###,###,###,###,###'
    end
    object queryGIRISLERSATIS_TUTARI: TFloatField
      FieldName = 'SATIS_TUTARI'
      Origin = 'ORTEG2002."stokhar.DB".SATIS_TUTARI'
      DisplayFormat = '###,###,###,###,###'
    end
    object queryGIRISLERKAR: TFloatField
      FieldName = 'KAR'
      Origin = 'ORTEG2002."stokhar.DB".KAR'
      DisplayFormat = '###,###,###,###,###'
    end
    object queryGIRISLERSTOK_KODU: TFloatField
      FieldName = 'STOK_KODU'
      Origin = 'ORTEG2002."stokhar.DB".STOK_KODU'
    end
    object queryGIRISLERNET_FIYAT: TFloatField
      FieldName = 'NET_FIYAT'
      Origin = 'ORTEG2002."stokhar.DB".NET_FIYAT'
    end
    object queryGIRISLERKDV: TFloatField
      FieldName = 'KDV'
      Origin = 'ORTEG2002."stokhar.DB".KDV'
    end
    object queryGIRISLERISKONTO: TFloatField
      FieldName = 'ISKONTO'
      Origin = 'ORTEG2002."stokhar.DB".ISKONTO'
    end
    object queryGIRISLERBIRIM_KATSAYISI: TFloatField
      FieldName = 'BIRIM_KATSAYISI'
      Origin = 'ORTEG2002."stokhar.DB".BIRIM_KATSAYISI'
    end
    object queryGIRISLERHAREKET_NO: TIntegerField
      FieldName = 'HAREKET_NO'
      Origin = 'ORTEG2002."stokhar.DB".HAREKET_NO'
    end
  end
  object queryCIKISLAR: TQuery
    DatabaseName = 'ORTEG2002'
    DataSource = datamoduleORTEG.dsrcUNITE
    SQL.Strings = (
      'select * from stokhar'
      
        'where (belge_turu='#39'CIKTI'#39' or belge_turu='#39'TEMIZLIK'#39') and firma_ad' +
        'i=:unite_adi'
      'order by tarih desc')
    Left = 137
    Top = 105
    ParamData = <
      item
        DataType = ftString
        Name = 'unite_adi'
        ParamType = ptUnknown
      end>
    object StringField1: TStringField
      FieldName = 'BELGE_TURU'
      Origin = 'ORTEG2002."stokhar.DB".BELGE_TURU'
      Size = 10
    end
    object FloatField1: TFloatField
      FieldName = 'FIRMA_KODU'
      Origin = 'ORTEG2002."stokhar.DB".FIRMA_KODU'
    end
    object StringField2: TStringField
      FieldName = 'FIRMA_ADI'
      Origin = 'ORTEG2002."stokhar.DB".FIRMA_ADI'
      Size = 35
    end
    object DateField1: TDateField
      FieldName = 'TARIH'
      Origin = 'ORTEG2002."stokhar.DB".TARIH'
    end
    object StringField3: TStringField
      FieldName = 'BELGE'
      Origin = 'ORTEG2002."stokhar.DB".BELGE'
      Size = 15
    end
    object StringField4: TStringField
      FieldName = 'IC_DAGITIM_NO'
      Origin = 'ORTEG2002."stokhar.DB".IC_DAGITIM_NO'
      Size = 15
    end
    object StringField5: TStringField
      FieldName = 'STOK_ADI'
      Origin = 'ORTEG2002."stokhar.DB".STOK_ADI'
      Size = 30
    end
    object StringField6: TStringField
      FieldName = 'BIRIMI'
      Origin = 'ORTEG2002."stokhar.DB".BIRIMI'
      Size = 15
    end
    object FloatField2: TFloatField
      FieldName = 'MIKTAR'
      Origin = 'ORTEG2002."stokhar.DB".MIKTAR'
    end
    object StringField7: TStringField
      FieldName = 'BIRIM2'
      Origin = 'ORTEG2002."stokhar.DB".BIRIM2'
      Size = 15
    end
    object FloatField3: TFloatField
      FieldName = 'MIKTAR2'
      Origin = 'ORTEG2002."stokhar.DB".MIKTAR2'
    end
    object FloatField4: TFloatField
      FieldName = 'ALIS_FIYATI'
      Origin = 'ORTEG2002."stokhar.DB".ALIS_FIYATI'
      DisplayFormat = '###,###,###,###,###'
    end
    object FloatField5: TFloatField
      FieldName = 'ALIS_TUTARI'
      Origin = 'ORTEG2002."stokhar.DB".ALIS_TUTARI'
      DisplayFormat = '###,###,###,###,###'
    end
    object FloatField6: TFloatField
      FieldName = 'SATIS_FIYATI'
      Origin = 'ORTEG2002."stokhar.DB".SATIS_FIYATI'
      DisplayFormat = '###,###,###,###,###'
    end
    object FloatField7: TFloatField
      FieldName = 'SATIS_TUTARI'
      Origin = 'ORTEG2002."stokhar.DB".SATIS_TUTARI'
      DisplayFormat = '###,###,###,###,###'
    end
    object FloatField8: TFloatField
      FieldName = 'KAR'
      Origin = 'ORTEG2002."stokhar.DB".KAR'
      DisplayFormat = '###,###,###,###,###'
    end
    object FloatField9: TFloatField
      FieldName = 'STOK_KODU'
      Origin = 'ORTEG2002."stokhar.DB".STOK_KODU'
    end
    object FloatField10: TFloatField
      FieldName = 'NET_FIYAT'
      Origin = 'ORTEG2002."stokhar.DB".NET_FIYAT'
    end
    object FloatField11: TFloatField
      FieldName = 'KDV'
      Origin = 'ORTEG2002."stokhar.DB".KDV'
    end
    object FloatField12: TFloatField
      FieldName = 'ISKONTO'
      Origin = 'ORTEG2002."stokhar.DB".ISKONTO'
    end
    object FloatField13: TFloatField
      FieldName = 'BIRIM_KATSAYISI'
      Origin = 'ORTEG2002."stokhar.DB".BIRIM_KATSAYISI'
    end
    object IntegerField1: TIntegerField
      FieldName = 'HAREKET_NO'
      Origin = 'ORTEG2002."stokhar.DB".HAREKET_NO'
    end
  end
end
